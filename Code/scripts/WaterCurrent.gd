...
@export var audio_prefab :PackedScene = preload("res://Scenes/Objects/_Spawnable/flow.tscn")
...
func _ready():
	...
	if Engine.is_editor_hint():
		...
		if (current_audio_instance==null) and audio_prefab!=null:
		...
		if visualizer:
			...
			visualizer.queue_free()
			visualizer=null
		if current_direction != Vector3() and show_wave and wave_instance !=null:
...
func _physics_process(delta):
	for body in bodies_affected:
		...
		body.apply_force(body.mass*current_direction*basis*current_power)
